...
def train(task_id, Neighbors, discriminator, generator, data, seen_classes, novel_classes, replay_feature, replay_label,
    ...
    for epoch in range(epochs):
        ...
        for feature, label in train_dataloader:
            ...
            seen_attr_proj = discriminator(seen_attr) # Shape: (10, 2048)
            mse_d = mse_loss(avg_feature, seen_attr_proj)

            # Compute discriminator classification loss
            batch_repeated_seen_attr_proj = discriminator(batch_repeated_seen_attr) # Shape: (128, 10, 2048)
            batch_repeated_seen_attr_proj_norm = F.normalize(batch_repeated_seen_attr_proj, p=2, dim=-1, eps=1e-12)
            class_repeated_feature_norm = feature_norm.unsqueeze(1).repeat([1, total_seen_classes, 1]) # Shape: (128, 10, 2048)
            real_cosine_sim = lambda1 * F.cosine_similarity(batch_repeated_seen_attr_proj_norm, class_repeated_feature_norm, dim=-1) # Shape: (128, 10)
            label = label.type(torch.LongTensor) # Added by Tauseef 
            label = label.to('cuda') # Added by Tauseef
            classification_losses = entropy_loss(real_cosine_sim, label.squeeze()) # Squeezed label shape: (128)

            d_loss = alpha * classification_losses + mse_d
...
def main(opt):
    ...
    for task_id in range(1, iter_task):
        avg_feature = train(task_id, opt.Neighbors, discriminator, generator,

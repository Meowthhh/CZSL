...
from dk_classifier import DKClassifier
...
def train(task_id, Neighbors, model, data, seen_classes, novel_classes, feature_size=2048, dlr=0.005, batch_size=64,
    ...
    D_optimizer = optim.Adam(model.parameters(), lr=dlr, weight_decay=0.00001)
    ...
    for epoch in tqdm(range(epochs), desc="Training Progress"):
        ...
        for feature, label in tqdm(train_dataloader, desc=f"Epoch {epoch+1}/{epochs}", leave=False):
            ...
            # model regularization loss
            # seen_attr_proj = model(seen_attr) # Shape: (10, 2048)
            # mse_d = mse_loss(avg_feature, seen_attr_proj)

            # Compute model classification loss
            ...
            d_loss = alpha * classification_losses
...
def main(opt):
    data = dataloader(opt)
    # model = DKClassifier(img_size=64).cuda()
    model = DKClassifier2().cuda()
    # model = Discriminator(data.att_size, data.feature_size).cuda()
    avg_feature = None
